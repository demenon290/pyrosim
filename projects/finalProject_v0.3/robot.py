...
class ROBOT:
	...
	def send_objects(self, sim):
		...
		#---OBJECTS---#		
		# gray base
		self.O0 = sim.send_box(x=0, y=0, z=c.L+c.R+c.Z, length=c.L, width=c.L, height=c.R*2, r=0.5, g=0.5, b=0.5)		
		# red leg y>0; r2>0
		self.O1 = sim.send_cylinder(x=0.0, y=c.L, z=c.L+c.R+c.Z,	 
								   length=c.L, radius=c.R, 	
								   r1=0, r2=1, r3=0, 		
								   r=1, g=0, b=0)			
		# green leg x>0; r1>0
		self.O2 = sim.send_cylinder(x=c.L, y=0.0, z=c.L+c.R+c.Z, 
								   length=c.L, radius=c.R, 
								   r1=1, r2=0, r3=0, 
								   r=0, g=1, b=0)
		# blue leg y<0; r2<0
		self.O3 = sim.send_cylinder(x=0.0, y=-c.L, z=c.L+c.R+c.Z, 
								   length=c.L, radius=c.R, 
								   r1=0, r2=-1, r3=0, 
								   r=0, g=0, b=1)
		# purple leg x<0; r1<0
		self.O4 = sim.send_cylinder(x=-c.L, y=0.0, z=c.L+c.R+c.Z, 
								   length=c.L, radius=c.R, 
								   r1=-1, r2=0, r3=0,
								   r=1, g=0, b=1)
		
		# red leg y>0; r3<0
		self.O5 = sim.send_cylinder(x=0.0, y=(c.L+c.L/2), z=(c.L+c.R)-(c.L/2)+c.Z, 
								   length=c.L, radius=c.R, 
								   r1=0, r2=0, r3=-1, 
								   r=1, g=0, b=0)
		# green leg x>0; r3<0
		self.O6 = sim.send_cylinder(x=(c.L+c.L/2), y=0.0, z=(c.L+c.R)-(c.L/2)+c.Z, 
								   length=c.L, radius=c.R, 
								   r1=0, r2=0, r3=-1, 
								   r=0, g=1, b=0)
		# blue leg y<0; r30
		self.O7 = sim.send_cylinder(x=0.0, y=-(c.L+c.L/2), z=(c.L+c.R)-(c.L/2)+c.Z, 
								   length=c.L, radius=c.R, 
								   r1=0, r2=0, r3=-1, 
								   r=0, g=0, b=1)
		# purple leg x<0; r3<0
		self.O8 = sim.send_cylinder(x=-(c.L+c.L/2), y=0.0, z=(c.L+c.R)-(c.L/2)+c.Z, 
								   length=c.L, radius=c.R, 
								   r1=0, r2=0, r3=-1, 
								   r=1, g=0, b=1)
